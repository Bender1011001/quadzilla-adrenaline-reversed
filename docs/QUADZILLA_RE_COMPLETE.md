...
## 12. What I Got Wrong and What I'd Do Differently <a name="mistakes"></a>

### Ghidra Iteration Was Too Slow
I wrote 6 different Ghidra scripts before getting one that worked. Each run took 3-5 minutes. I should have written a standalone Python disassembler for Thumb code first to understand the binary structure, then written ONE Ghidra script with that knowledge. Instead I used Ghidra as both the analysis tool and the validation tool, which made the feedback loop painfully slow.

### I Over-Relied on Auto-Analysis
My first instinct was to trust Ghidra's auto-analyzer and just fix the edges. This wasted hours. The lesson: for embedded firmware with non-standard ISA switching, auto-analysis is worse than useless — it actively makes the problem harder by classifying code as data, which then blocks manual analysis.

### I Should Have Started with the .so Library
The x86_64 `libx2com-jni.so` was trivial to decompile and gave me the complete protocol. If I'd done that first, I would have understood the message format before trying to understand the firmware's protocol handler (FUN_5CF0), which would have made the firmware analysis faster.
