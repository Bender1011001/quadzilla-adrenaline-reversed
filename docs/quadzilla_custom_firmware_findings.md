...
- Checks for ‘B’, ‘Z’, ‘h’ in sequence:
  - ‘B’ test: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L130)
  - ‘Z’ test: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L151)
  - ‘h’ test: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L172)
- Then reads a character and enforces ‘0’..‘9’ as the level digit (bzip2 block size digit):
  - Digit path: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L195)
- The sequence “1 AY&SY” (part of bzip2 block header) cascades through subsequent character checks as the state advances:
  - Branch chain immediately after header: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L236)
- Bit-buffered input, classic for bzip2:
  - Repeated “fill at least N bits” loops: e.g., [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L367)
- Huffman decode table construction and symbol-frequency accumulation, consistent with bzip2 canonical Huffman routines and MTF/BWT:
  - Table-building logic: [FUN_0042c706()](../analysis/decomp_FUN_0042c706.c#L5)
  - Symbol map consolidation: [FUN_004292a0()](../analysis/decomp_FUN_004292a0.c#L4)
- Index and width checks aligning with code-length bounds and per-block decoding flows seen in bzip2 inflators:
  - Bound checks and tree-walk: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L1401)
...
- [FUN_0042c706()](../analysis/decomp_FUN_0042c706.c#L5): Constructs canonical Huffman decoding tables and prefix-sum count arrays used in bzip2 blocks. Note the symbol length histogramming, cumulative counts, and per-length bounds typical in canonical Huffman.
- [FUN_004292a0()](../analysis/decomp_FUN_004292a0.c#L4): Builds symbol lists (active byte values) used in the MTF/BWT step; it populates a compact indexing of symbols present in the block, again matching bzip2 behavior.
...
- overlay.bin is a bzip2-compressed payload extracted from the Windows updater executable `ADR9802v2.8.4.exe` using the repository script [extract_pe_overlay.py](../analysis/extract_pe_overlay.py#L1).
- Our regex scan for CRC-8 lookup tables over overlay.bin returned none, consistent with bzip2’s built-in per-block CRC (and trailing stream CRC) instead of an external CRC-8:
  - No CRC-8 tables found: [crc_table_hits_overlay.txt](../analysis/crc_table_hits_overlay.txt#L1)
...
- Extraction scripts present beside the project (you can use standard bzip2 tooling as well):
  - [extract_bzip_payloads.py](../analysis/extract_bzip_payloads.py#L1)
  - [extract_pe_overlay.py](../analysis/extract_pe_overlay.py#L1)
...
- Power levels: The repository already indicates a power-level concept (e.g., [autotune_state.json](../analysis/autotune_state.json#L1) shows “power_level”: 5). If the payload uses multiple maps/levels, you can alter these sets.
...
- A confirmed disassembly of the device’s microcontroller code that computes fueling and sends the CAN messages. The decompiled functions we examined such as [FUN_0041d527()](../analysis/decomp_FUN_0041d527.c#L4) and [FUN_004227b8()](../analysis/decomp_FUN_004227b8.c#L5) are in the Windows updater, not MCU firmware. In fact:
  - [FUN_0041d527()](../analysis/decomp_FUN_0041d527.c#L4) is a standard printf/formatting-like implementation (format flags, width/precision, length modifiers, cases for %d/%x/%s/%c), not a sensor parser.
  - [FUN_004227b8()](../analysis/decomp_FUN_004227b8.c#L5) is a numeric parser akin to strtod/decimal exponent handling, also PC-side utility code.
...
- Step A: Decompress overlay.bin
  - Use any standard bzip2 tool or Python’s bz2 module to decompress:
    - The “BZh” header is confirmed by code checks at [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L130), [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L151), [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L172).
  - Alternatively, adapt [extract_bzip_payloads.py](../analysis/extract_bzip_payloads.py#L1).
...
- Step D: Repack as bzip2
  ...
  - Our CRC scan for overlay.bin showed no external CRC-8 LUTs:
    - [crc_table_hits_overlay.txt](../analysis/crc_table_hits_overlay.txt#L1)
...
- Switch/dispatcher enumeration shows our “state machine” function with many cases, but these cases simply step through bzip2 bitstream parsing states:
  - [aid_switch_candidates.txt](../analysis/aid_switch_candidates.txt#L3)
  - [aid_dispatch_map.txt](../analysis/aid_dispatch_map.txt#L1)
...
- Headless Ghidra run was performed against the Windows executable to sanity-check function call relationships and highlight “potential” parser/utility functions. Our headless post-script identified printf-like and numeric parsing functions as callers/callees that live in the PC app, not on the device:
  - Script used: [find_table_references.py](../analysis/find_table_references.py#L1)
  - Example callers:
    - FUN_0041d527 called from FUN_0041a9e4 and FUN_0041acde in the PE (output excerpt captured during headless run).

- The presence of bzip2 “1AY&SY” block magic was evident in the state chain (ASCII spelled across the subsequent case checks in [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L236)), which is an exact bzip2 block header marker (see bzip2 sources below).
...
- bzip2 state machine (Windows updater): [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L1)
  - “B” check: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L130)
  - “Z” check: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L151)
  - “h” check: [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L172)
  - post-header chain (matches block magic “1AY&SY”): [decomp_FUN_00427401.c](../analysis/decomp_FUN_00427401.c#L236)
- Huffman table builder: [FUN_0042c706()](../analysis/decomp_FUN_0042c706.c#L5)
- Symbol list/active set builder (used by MTF/BWT step): [FUN_004292a0()](../analysis/decomp_FUN_004292a0.c#L4)
- printf-like formatter (PC-side): [FUN_0041d527()](../analysis/decomp_FUN_0041d527.c#L4)
- numeric parser akin to strtod (PC-side): [FUN_004227b8()](../analysis/decomp_FUN_004227b8.c#L5)
- CRC-8 LUT scan result: [crc_table_hits_overlay.txt](../analysis/crc_table_hits_overlay.txt#L1)
- overlay payload container: `overlay.bin`
- PE updater containing overlay: `ADR9802v2.8.4.exe`
- Utility scripts:
  - [extract_bzip_payloads.py](../analysis/extract_bzip_payloads.py#L1)
  - [extract_pe_overlay.py](../analysis/extract_pe_overlay.py#L1)
  - [find_table_references.py](../analysis/find_table_references.py#L1)
